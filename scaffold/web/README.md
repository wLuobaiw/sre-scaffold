...
| 配置项 | 环境变量 | 默认值 | 说明 |
|--------|---------|--------|------|
| `SECRET_KEY` | `SECRET_KEY` | 开发用固定值 | Flask secret key |
| `DEBUG` | `DEBUG` | true | 调试模式 |
| `MATERIALS_REPO_URL` | `MATERIALS_REPO_URL` | `""`（空） | Git 仓库 URL，留空使用 co-located toolkit |
| `MATERIALS_LOCAL_PATH` | `MATERIALS_LOCAL_PATH` | `/app/toolkit` | 物料仓库本地路径（容器内） |
| `REGISTRY_URL` | `REGISTRY_URL` | registry.example.com | 镜像仓库地址 |
| `ANSIBLE_PLAYBOOK_BIN` | `ANSIBLE_PLAYBOOK_BIN` | ansible-playbook | Ansible 可执行文件路径 |
...
| 文件 | 职责 | 状态 |
|------|------|------|
| `ansible.py` | ansible-runner 封装。`run_playbook(playbook, hosts, extra_vars)` → 生成日志回调 | 预留 |
| `gitops.py` | 物料仓库操作。`ensure_repo()` 支持 co-located 和远程两种模式；`scan_components()` 扫描 roles 目录 | 已实现 |
| `registry.py` | 镜像仓库操作。`pull_image()` / `distribute_to_hosts()` 拉取镜像分发到目标机 | 预留 |
| `stack.py` | YAML 栈管理。`load_stack()` / `save_stack()` / `generate_inventory()` 生成 Ansible inventory | 预留 |

## 启动

```bash
cd path/to/sre-scaffold/scaffold/web
